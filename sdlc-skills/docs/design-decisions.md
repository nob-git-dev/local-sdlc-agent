...
ある自社プロダクトの開発で AI エージェントを用いた作業中に、
...
| エージェント向け指示ファイルにルール追記 | 読んで守るかは実行者次第 |
...
**根拠**: 対象とした実行環境では、サブエージェントから別のサブエージェントを生成できない。
...
- 利用するエージェント実行環境の Skills・Subagents・複数エージェント機能に関する公式仕様
