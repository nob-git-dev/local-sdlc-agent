#!/bin/bash
# Local Learning Skills インストールスクリプト
#
# このスクリプトは skills/（3つの学習パイプラインスキル）をエージェント設定ディレクトリに展開します。
# 同名のスキルが既にある場合は上書きされるため、事前に自動バックアップを取ります。
# エージェント向け指示ファイル・settings.json・他のスキルには一切触れません。
...
echo "Local Learning Skills インストーラー"
echo "===================================="
echo "リポジトリ: $REPO_DIR"
echo "インストール先: $AGENT_CONFIG_DIR/skills"
echo ""
...
NEED_BACKUP=0
for s in $SKILLS ; do
  [ -d "$AGENT_CONFIG_DIR/skills/$s" ] && NEED_BACKUP=1
done
if [ "$NEED_BACKUP" = "1" ]; then
  BACKUP_DIR="$AGENT_CONFIG_DIR/.backup-learning-$(date +%Y%m%d-%H%M%S)"
  echo "同名の既存スキルを $BACKUP_DIR にバックアップします..."
  mkdir -p "$BACKUP_DIR"
  for s in $SKILLS ; do
    [ -d "$AGENT_CONFIG_DIR/skills/$s" ] && cp -r "$AGENT_CONFIG_DIR/skills/$s" "$BACKUP_DIR/"
  done
  echo "バックアップ完了"
  echo ""
fi
...
# インストール
mkdir -p "$AGENT_CONFIG_DIR/skills"
echo "学習パイプラインスキルをインストール中..."
for s in $SKILLS ; do
  cp -r "$REPO_DIR/skills/$s" "$AGENT_CONFIG_DIR/skills/"
  echo "  - $s"
done
...
echo "対応するエージェント実行環境の次回起動時から、以下で呼び出せます。"
echo ""
